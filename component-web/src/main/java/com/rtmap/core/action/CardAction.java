...
import com.rtmap.wx.sdk.constant.CardTypeEnum;
...
public class CardAction extends AbstractAction {
    ...
    public Result create(@PathVariable("authAppid") String authAppid, CardParam param, HttpServletRequest request) {
        ...
        try {
            ...
            return ResponseFactory.buildMessage(Code.NULL_PARAM, e.getMessage());
    ...
    private MCard _mountCard(CardParam param, String authAppid) throws IOException {
        ...
        CardTypeEnum cardTypeEnum = CardTypeEnum.get(param.getCard_type());
        assert cardTypeEnum != null;
        switch (cardTypeEnum) {
            case GROUPON:
                mcard.setCard_type(CardTypeEnum.GROUPON.getCode());
                mcard.setDefault_detail(param.getDefault_detail());
                break;
            case CASH:
                mcard.setCard_type(CardTypeEnum.CASH.getCode());
                mcard.setLeast_cost(param.getLeast_cost());
                mcard.setReduce_cost(param.getReduce_cost());
                break;
            case DISCOUNT:
                mcard.setCard_type(CardTypeEnum.DISCOUNT.getCode());
                mcard.setDiscount(param.getDiscount());
                break;
            case GIFT:
                mcard.setCard_type(CardTypeEnum.GIFT.getCode());
                mcard.setGift(param.getGift());
                break;
            case GENERAL_COUPON:
                mcard.setCard_type(CardTypeEnum.GENERAL_COUPON.getCode());
                mcard.setDefault_detail(param.getDefault_detail());
                break;
        }
