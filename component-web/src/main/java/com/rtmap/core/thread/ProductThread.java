...
/**
 * 
 * ProductThread. HTTP协议推送事件给消息队列中间层
 * 
 * @author muarine
 * @since 2.2.4
 */
public class ProductThread implements Runnable{
	 ...
	 */
	public ProductThread(String notify_url , MultiValueMap<String,Object> multiValueMap) {
		super();
	...
	public void run() {
		...
		for (int i = 0; i < 3; i++) {
			try {
				...
				if(JSON.parseObject(result).getInteger("status") == 200){
					break;
				}
			} catch (Exception e) {
