 ...
 */
package com.rtmap.utils.thread;
...
public class NotifyThread implements Runnable{
	...
	private static final Logger log = LoggerFactory.getLogger(NotifyThread.class);
	...
	public void run() {
		log.info("消息推送线程开始执行");
		log.info("notify_url:{} , request_params:{}" , notify_url , JSON.toJSONString(multiValueMap));
		for (int i = 0; i < 3; i++) {
			try {
				...
				log.info("异步通知返回结果:{}" , result);
				if(JSON.parseObject(result).getInteger("code") == 200){
					break;
				}
			} catch (Exception e) {
				log.error(e.getMessage(),e);
				log.info("正在建立重试机制,重新尝试请求通知接口...");
			}
			if(i >= 2){
				log.error("异步通知失败，重试次数:{}" , i);
			}
		}
